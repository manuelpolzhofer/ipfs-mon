...
func init() {
	...
	rootCmd.PersistentFlags().IntVar(&bits, "bits", defaultBits, "define the n-bit zone for a crawl. (0-256)")
	rootCmd.PersistentFlags().IntVar(&workers, "workers", defaultWorker, "define the amount of workers per node for the crawl")
	rootCmd.PersistentFlags().IntVar(&nodes, "nodes", defaultNodes, "define the amount of ipfs nodes for the crawl")
	rootCmd.PersistentFlags().IntVar(&maxPeers, "maxPeers", defaultMaxPeers, "stop crawling after maxPeers are found in zone")
}
